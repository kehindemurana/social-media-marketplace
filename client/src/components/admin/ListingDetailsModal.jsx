...
const ListingDetailsModal = ({ listing, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/70 z-[100]
         backdrop-blur flex items-center justify-center sm:p-4">
            <div className="bg-white sm:rounded-lg w-full max-w-2xl
             h-screen sm:h-[600px] flex flex-col overflow-hidden">
                {/* Header */}
                <div className="bg-linear-to-r from-indigo-600
                 to-indigo-400 text-white p-4 sm:rounded-t-lg 
                ...
                <div className="p-5 overflow-y-auto space-y-5 text-gray-700">
                    ...
                    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3
                     overflow-x-auto">
                        {listing.images?.map((img, i) => (
                            <img key={i} src={img} alt={`${listing.title}-${i}`}
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        <span className={`px-3 py-1 rounded-full text-xs font-semibold
                             ${listing.status === "active" ? "bg-green-100 text-green-600"
                              : 
                         "bg-gray-100 text-gray-500"}`}>{listing.status?.toUpperCase()}</span>
                        {listing.verified && (
                            <span className="flex items-center gap-1 text-indigo-600
                             text-xs font-semibold bg-indigo-100 px-2 py-1 rounded-full">
                                <CheckCircleIcon size={14} /> Verified
                            </span>
                        )}
                        {listing.monetized && <span className="text-xs font-semibold
                         bg-yellow-100 text-yellow-600 px-2 py-1 
                         rounded-full">Monetized</span>}
                        {listing.featured && <span className="text-xs font-semibold
                         bg-purple-100 text-purple-600 px-2 py-1
                          rounded-full">Featured</span>}
                        {listing.platformAssured && <span className="text-
                         font-semibold bg-cyan-100 text-cyan-600 px-2 py-1
                          rounded-full">Platform Assured</span>}
                    </div>
                    ...
                    {listing.owner && (
                        <div className="border-t border-gray-200 pt-3 mt-3">
                            ...
                            <div className="flex items-center gap-3">
                                <img src={listing.owner.image} alt={listing.owner.name}
                    ...
                    {listing.customer && (
                        <div className="border-t border-gray-200 pt-3 mt-3">
                            ...
                            <div className="flex items-center gap-3">
                                <img src={listing.customer.image} 
                                alt={listing.customer.name} className="size-8 

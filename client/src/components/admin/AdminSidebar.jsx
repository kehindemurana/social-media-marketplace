...
const AdminSidebar = () => {
    ...
    return (
        ...
        items-center pt-8 max-w-13 md:max-w-60 w-full border-r border-gray-200
         text-sm'>
            <img className='size-9 md:size-12 rounded-full
             mx-auto' src={user.imageUrl} alt="sidebar" />
            ...
            <div className='w-full'>
                {adminNavlinks.map((link, index) => (
                    ...
                    className={({ isActive }) => `relative flex items-center
                     max-md:justify-center gap-2 w-full py-2.5 md:pl-10 
                     first:mt-6 text-gray-600 ${isActive &&
                      'bg-indigo-500/10 text-indigo-600 group'}`}>
                        {({ isActive }) => (
                            <>
